...
void Custom::RobotControl() 
{
    ...
    if( motiontime >= 0){
        ...
        if( motiontime >= 10 && motiontime < 400){
            ...
            qDes[0] = jointLinearInterpolation(qInit[0], sin_mid_FR[0], rate);
            qDes[2] = jointLinearInterpolation(qInit[2], sin_mid_FR[2], rate);
...
////////////////////////////////////////////
    ...
    if( motiontime >= 500){
        ...
        dqInit[1] = state.motorState[FR_1].dq;

        if (dqInit[1] >= 0)
        {
            _comp = _Kcomp * dqInit[1];
        }
        else
        {
            _comp = _Kcomp * dqInit[1];
        ...
        float torque = (0 - state.motorState[FR_1].q)*5.0f + (0 - state.motorState[FR_1].dq)*0.0f;
